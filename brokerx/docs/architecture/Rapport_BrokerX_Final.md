...
- Dashboards Grafana versionnés (`docs/architecture/observability`) : Golden Signals, matching/notifications, panels outbox. Grafana sur `http://localhost:3001`, Prometheus sur `http://localhost:9090`.
...
- **Observabilité** : Micrometer Prometheus, Grafana dashboards (`docs/architecture/observability`). Grafana port 3001, Prometheus port 9090.
...
- **Résultats charge (5 VU, 30 s)** : Gateway (8081) p90 ~99 ms, p95 ~183 ms (~8.6 req/s, 0 % erreurs) ; Direct api1 (8085) p90 ~83 ms, p95 ~182 ms (~8.7 req/s, 0 % erreurs). Voir `docs/architecture/observability/README.md` et Grafana (Golden Signals).
