...
public class ModifyOrder {
  ...
  public OrderMutation replace(ReplaceCommand cmd) {
    ...
    publishOrderUpdated(order, OrderEvents.ORDER_UPDATED);
    return mutation;
  }
  ...
  public OrderMutation cancel(CancelCommand cmd) {
    ...
    orderCacheService.evictForOrder(order.getId());
    publishOrderUpdated(order, OrderEvents.ORDER_CANCELED);
    return mutation;
  }
  ...
  private void publishOrderUpdated(OrderEntity order, String topic) {
    try {
      String payload =
          """
          {"orderId":"%s","accountId":"%s","status":"%s","version":%d,"qty":%d,"limitPrice":%s}
          """
              .formatted(
                  order.getId(),
                  order.getAccountId(),
      ...
      // ne bloque pas le flux en cas d'échec d'émission
    }
  }
}
