...
public class PlaceOrder {
  ...
  public Ack handle(Draft draft) {
    ...
    Ack ack = new Ack(order.getId(), order.getStatus());
    auditLogger.record(
        "ORDER",
        "ORDER_ACCEPTED",
        order.getAccountId(),
  ...
  private void publishOrderPlaced(OrderEntity order) {
    try {
      String payload =
          """
          {"orderId":"%s","accountId":"%s","side":"%s","type":"%s","symbol":"%s","qty":%d,"limitPrice":%s}
          """
              .formatted(
                  order.getId(),
                  order.getAccountId(),
      ...
      eventPublisher.publish(OrderEvents.ORDER_PLACED, payload);
    } catch (Exception e) {
